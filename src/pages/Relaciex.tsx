 ...
 * - Compatível com Vite + React + TS no Vercel
 * - Toda a lógica anterior convertida para hooks/useEffect
 */
const LiveClass: React.FC = () => {
  ...
  // Para testes rápidos, substitua o objeto acima por:
  ...
  // Viewers e online (com aceleração quando o usuário interage)
  useEffect(() => {
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      ...
      const pick = [...cities].sort(() => 0.5 - Math.random()).slice(0, 3);
      const c1 = document.getElementById("city1");
      const c2 = document.getElementById("city2");
      const c3 = document.getElementById("city3");
      if (c1) c1.textContent = `${pick[0]} ${actions[Math.floor(Math.random() * actions.length)]}`;
      if (c2) c2.textContent = `${pick[1]} ${actions[Math.floor(Math.random() * actions.length)]}`;
      if (c3) c3.textContent = `${pick[2]} ${actions[Math.floor(Math.random() * actions.length)]}`;
    }, 8000);
    intervalsRef.current.push(id);
    return () => window.clearInterval(id);
  }, [cities]);
      ...
      <div className="main-container">
        ...
        <div className="video-section">
          ...
          <div className="video-placeholder" onClick={handleVideoClick}>
            <div>▶️</div>
            {!ctaActivated && (
              <div className="question-overlay" id="questionOverlay">
                💬 Responda no chat: Qual sua maior dificuldade com <strong>o tema da aula</strong>?
              </div>
            )}
            <div className="video-timer" id="videoTimer">
        ...
        <div className="activity-indicators">
          <div className="indicator">
            ...
            <span id="city1">São Paulo, SP está assistindo</span>
          </div>
          <div className="indicator">
            <div className="indicator-icon" />
            <span id="city2">Rio de Janeiro, RJ entrou agora</span>
          </div>
          <div className="indicator">
            <div className="indicator-icon" />
            <span id="city3">Fortaleza, CE está online</span>
          </div>
        </div>
      ...
      <div className={`ticker ${tickerActive ? "active" : ""}`} id="ticker">
        <div className="ticker-track">
          <div
            ...
            id="tickerContent"
            dangerouslySetInnerHTML={{ __html: tickerItems.map((i) => `<span class="ticker-item">${i}</span>`).join("") }}
          />
        </div>
      </div>
